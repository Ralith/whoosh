...
data Distribution
    ...
    | LogNormal Rational Rational Rational
    | Gibbs [(Rational, Distribution)]

mean :: Floating a => Distribution -> a
mean (Normal mu _) = fromRational mu
mean (LogNormal mu sigma scale) = (fromRational scale) * exp ((fromRational mu) + (fromRational sigma)**2/2)
...
median (LogNormal mu _ scale) = (fromRational scale) * exp (fromRational mu)
...
pdf (LogNormal mu sigma scale) x
    = exp (- (log (x/(fromRational scale)) - (fromRational mu))**2/(2*(fromRational sigma)**2))
      / (x/(fromRational scale) * sqrt (2*pi*(fromRational sigma)**2))
...
sample (LogNormal mu sigma scale) g
    = let (x, g') = boxMuller g in
      ((fromRational scale) * exp ((fromRational mu) + (fromRational sigma) * x), g')
sample (Gibbs ds) g
